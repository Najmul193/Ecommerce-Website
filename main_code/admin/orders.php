...
include 'header.php'; ?>
        <div class="admin-content-container">
            ...
            $limit = 5;
            $db = new Database();
            $db->sql('SELECT order_products.product_id,order_products.order_id,order_products.total_amount,order_products.product_qty,order_products.product_user,order_products.order_date,products.featured_image,user.f_name,user.address,user.city FROM order_products LEFT JOIN products ON FIND_IN_SET(products.product_id,order_products.product_id) > 0
                     LEFT JOIN user ON order_products.product_user=user.user_id GROUP BY order_products.order_id ORDER BY order_products.order_id DESC');
                $result = $db->getResult();
                if(count($result) > 0) {  ?>
                ...
                <div class="pagination-outer">
                    ...
                    LEFT JOIN user ON order_products.product_user=user.user_id',null,$limit); ?>
                </div>
        </div>
<?php
//    include footer file
