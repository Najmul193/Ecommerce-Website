...
include 'header.php'; ?>
    <div class="admin-content-container">
        ...
        $limit = 10;
        $db = new Database();
        $db->select('products','products.product_id,products.product_cat,products.product_sub_cat,products.product_brand,product_title,products.product_price,products.qty,products.product_status,products.featured_image,sub_categories.sub_cat_title,brands.brand_title','sub_categories ON products.product_sub_cat=sub_categories.sub_cat_id LEFT JOIN brands ON products.product_brand=brands.brand_id',null,'products.product_id DESC',$limit);
        $result = $db->getResult();
        if (count($result) > 0) { ?>
